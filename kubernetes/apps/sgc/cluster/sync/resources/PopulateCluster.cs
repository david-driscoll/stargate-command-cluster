...
using AuthentikSpec = Models.Authentik.AuthentikSpec;
...
where TResult : KubernetesObject, IMetadata<V1ObjectMeta>
{
  ...
  {
    await localCluster.CustomObjects.CreateNamespacedCustomObjectAsync(missingEntity, group, version, destinationNamespace, plural);
  }

  foreach (var removedEntity in removedRemoteEntities)
  {
    await localCluster.CustomObjects.DeleteNamespacedCustomObjectAsync(group, version, destinationNamespace, plural, removedEntity.Metadata.Name);
  }
}
...
static async ValueTask<ApplicationDefinition> MapApplicationDefinition(Kubernetes remoteCluster, string cluster, ApplicationDefinition resource)
{
  ...
  if (spec is { AuthentikFrom: { } authentikFrom })
  {
    ...
    {
      var configMap =
        await remoteCluster.CoreV1.ReadNamespacedConfigMapAsync(configMapName, resource.Namespace());
      data = configMap.Data;
    }
    else if (authentikFrom is { Type: "secret", Name: var secretName })
    {
      var secret = await remoteCluster.CoreV1.ReadNamespacedSecretAsync(secretName, resource.Namespace());
...
static KumaResource MapKumaEntity(string cluster, KumaResource resource)
{
  var prefix = cluster;
