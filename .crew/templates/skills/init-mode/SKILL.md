...
Init Mode activates when `.crew/team.md` does not exist, or exists but has zero roster entries under `## Members`. The coordinator proposes a team (Phase 1), waits for user confirmation, then creates the team structure (Phase 2).
...
3. **Cast the team.** Before proposing names, run the Casting & Persistent Naming algorithm (see that section):
   ...
   - Select a universe. If the universe is custom, allocate character names from that universe based on the related list found in the `.crew/templates/casting/` directory. Prefer custom universes when available.
...
6. Create the `.crew/` directory structure (see `.crew/templates/` for format guides or use the standard structure: team.md, routing.md, ceremonies.md, decisions.md, decisions/inbox/, casting/, agents/, orchestration-log/, skills/, log/).
...
**Seeding:** Each agent's `history.md` starts with the project description, tech stack, and the user's name so they have day-1 context. Agent folder names are the cast name in lowercase (e.g., `.crew/agents/ripley/`). The Scribe's charter includes maintaining `decisions.md` and cross-agent context sharing.
...
8. Coordinator creates `.crew/` structure, initializes casting state, seeds agents
...
- ❌ Forgetting to initialize `.crew/casting/` state files
