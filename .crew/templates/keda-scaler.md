...
When running Crew on Kubernetes, agent pods sit idle when no work exists. [KEDA](https://keda.sh) (Kubernetes Event-Driven Autoscaler) solves this for queue-based workloads, but GitHub Issues isn't a native KEDA trigger.

The `keda-copilot-scaler` is a KEDA External Scaler (gRPC) that bridges this gap:
1. Polls GitHub API for issues matching specific labels (e.g., `crew:copilot`)
...
helm install keda-copilot-scaler oci://ghcr.io/tamirdresher/keda-copilot-scaler \
  --namespace crew-scaler --create-namespace \
...
spec:
  ...
  - type: external
    metadata:
      scalerAddress: keda-copilot-scaler.crew-scaler.svc.cluster.local:6000
      owner: your-org
      repo: your-repo
      labels: crew:copilot    # Only count issues with this label
...
kubectl get pods -n crew-scaler

# Check ScaledObject status
kubectl get scaledobject picard-scaler -n crew

# Watch scaling events
kubectl get events -n crew --watch
```
...
spec:
  template:
    spec:
      nodeSelector:
        node.crew.dev/gpu: "true"
  triggers:
  - type: external
    metadata:
      labels: crew:copilot,needs:gpu
```
...
| `github.labels` | `crew:copilot` | Comma-separated label filter |
...
- [Crew on AKS](https://github.com/tamirdresher/crew-on-aks) — Full Kubernetes deployment example
