...
When running Crew across multiple machines (laptops, DevBoxes, GPU servers, Kubernetes nodes), each machine has different tooling. The capability system lets you declare what each machine can do, and Ralph automatically routes work accordingly.

## Setup

### 1. Create a Capabilities Manifest

Create `~/.crew/machine-capabilities.json` (user-wide) or `.crew/machine-capabilities.json` (project-local):
...
A DaemonSet can run capability discovery on each node and maintain labels automatically. See the [crew-on-aks](https://github.com/tamirdresher/crew-on-aks) project for a complete Kubernetes deployment example.