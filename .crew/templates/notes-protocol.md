...
> here instead of modifying `.crew/` files in PRs.
>
> **Version:** 1.0
...
|-----------|-------|---------|
| `refs/notes/crew/data` | Data | Architecture decisions, implementation choices |
| `refs/notes/crew/worf` | Worf | Security reviews, vulnerability assessments |
| `refs/notes/crew/seven` | Seven | Documentation quality, API contract decisions |
| `refs/notes/crew/ralph` | Ralph | Work-round progress, task-state annotations |
| `refs/notes/crew/q` | Q | Devil's advocate findings, risk assessments |
| `refs/notes/crew/research` | Any agent | Research notes that should survive branch deletion |
| `refs/notes/crew/review` | Any agent | Code review context (mirrors Gerrit's pattern) |
...
git notes --ref=crew/{your-agent} append \
  -m '{"agent":"{Agent}","timestamp":"...","type":"progress","content":"..."}' \
  HEAD

# Read your note
git notes --ref=crew/{your-agent} show HEAD

# List all commits with notes in your namespace
git notes --ref=crew/{your-agent} list
```
...
1. **Per-agent namespaces prevent 99% of conflicts.** Only one agent writes to
   `refs/notes/crew/data`, so there are no write conflicts in normal use.

2. **Same agent, two machines:** First push wins. Losing machine should fetch
   and append:
   ```bash
   git fetch origin 'refs/notes/*:refs/notes/*'
   git notes --ref=crew/{agent} append -m '{...}' HEAD
...
4. **Push conflict recovery:**
   ```bash
   git fetch origin 'refs/notes/*:refs/notes/*'
   git notes merge refs/notes/remotes/origin/crew/{namespace}
   git push origin 'refs/notes/*:refs/notes/*'
   ```
...
1. List notes in `crew/research` on the closed branch's commits
