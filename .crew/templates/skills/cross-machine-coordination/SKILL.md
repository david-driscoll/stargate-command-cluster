...
description: "Enables crew agents on different machines to share work via git-based task queuing"
...
Enables crew agents running on different machines (laptop, DevBox, Azure VM) to securely share work, coordinate execution, and pass results without manual intervention.
...
```bash
# Create task file
cat > .crew/cross-machine/tasks/2026-03-14T1530Z-laptop-gpu-voice-clone.yaml << 'EOF'
...
# Commit & push
git add .crew/cross-machine/tasks/
git commit -m "Cross-machine task: GPU voice cloning [crew:machine-devbox]"
...
4. Write result to `.crew/cross-machine/results/gpu-voice-clone-001.yaml`
...
Ralph automatically watches `.crew/cross-machine/tasks/` for work targeted at this machine.
...
2. Load all .yaml files in .crew/cross-machine/tasks/
...
4. For each task:
   ...
   d. Write result to .crew/cross-machine/results/{id}.yaml
...
**Use GitHub Issues with `crew:machine-{name}` label:**
...
gh issue create \
  ...
  --label "crew:machine-devbox" \
  --label "urgent"
```

Ralph on DevBox will:
1. Detect issue with `crew:machine-devbox` label
...
**Location:** `.crew/cross-machine/tasks/{timestamp}-{machine}-{task-id}.yaml`
...
**Location:** `.crew/cross-machine/results/{task-id}.yaml`
...
Ralph reads config from `.crew/config.json`:
...
```yaml
# .crew/cross-machine/tasks/2026-03-14T1530Z-laptop-gpu-001.yaml
id: gpu-voice-clone-001
source_machine: laptop-machine
target_machine: devbox
priority: high
...
```bash
git add .crew/cross-machine/tasks/
git commit -m "Task: GPU voice cloning [crew:machine-devbox]"
git push origin main
```
...
```yaml
# .crew/cross-machine/results/gpu-voice-clone-001.yaml
id: gpu-voice-clone-001
target_machine: devbox
...
```bash
ls -la .crew/cross-machine/tasks/
cat .crew/cross-machine/tasks/*.yaml | grep -E "^(id|status|target_machine):"
```

### Check Results

```bash
ls -la .crew/cross-machine/results/
cat .crew/cross-machine/results/{task-id}.yaml
```

### View Execution History

```bash
git log --oneline .crew/cross-machine/ | head -20
```

### Monitor Ralph Cycles

```bash
tail -f .crew/log/ralph-watch.log | grep "cross-machine"
```
...
1. Fetch GitHub issues with crew:machine-* labels
2. Poll .crew/cross-machine/tasks/
