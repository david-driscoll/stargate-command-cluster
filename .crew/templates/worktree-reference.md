...
Crew and all spawned agents may be running inside a **git worktree** rather than the main checkout. All `.crew/` paths (charters, history, decisions, logs) MUST be resolved relative to a known **team root**, never assumed from CWD.

**Two strategies for resolving the team root:**

| Strategy | Team root | State scope | When to use |
|----------|-----------|-------------|-------------|
| **worktree-local** | Current worktree root | Branch-local — each worktree has its own `.crew/` state | Feature branches that need isolated decisions and history |
| **main-checkout** | Main working tree root | Shared — all worktrees read/write the main checkout's `.crew/` | Single source of truth for memories, decisions, and logs across all branches |

**How the Coordinator resolves the team root (on every session start):**

0. **Check config.json overrides first** — read `.crew/config.json` in the current directory (or at the git root):
...
1. **Check CWD first** — does `.crew/` exist in the current working directory?
   - **Yes** → Team root = CWD. This handles monorepos where `.crew/` lives in a subfolder.
2. If not, run `git rev-parse --show-toplevel` to get the current worktree root.
3. Check if `.crew/` exists at that root (fall back to `.ai-team/` for repos that haven't migrated yet).
...
- Agents resolve ALL `.crew/` paths from the provided team root — charter, history, decisions inbox, logs.
...
- `.crew/` files are **branch-local**. Each worktree works independently — no locking, no shared-state races.
- When branches merge into main, `.crew/` state merges with them. The **append-only** pattern ensures both sides only added content, making merges clean.
...
- The Scribe commits `.crew/` changes to the worktree's branch. State flows to other branches through normal git merge / PR workflow.

**Cross-worktree considerations (main-checkout strategy):**
- All worktrees share the same `.crew/` state on disk via the main checkout — changes are immediately visible without merging.
...
- Explicit: `worktrees: true` in project config (crew.config.ts or package.json `crew` section)
- Environment: `CREW_WORKTREES=1` set in environment variables
...
- Path convention: `{repo-parent}/{repo-name}-{issue-number}`
  - Example: Working on issue #42 in `C:\src\crew` → worktree at `C:\src\crew-42`
- Branch: `crew/{issue-number}-{kebab-case-slug}` (created from base branch, typically `main`)
...
- Is `CREW_WORKTREES=1` set in the environment?
...
a. **Determine the worktree path:**
   ...
   - Example: Main repo at `C:\src\crew`, issue #42 → `C:\src\crew-42`
...
b. **Check if worktree already exists:**
   ...
   - If the worktree path already exists → **reuse it**:
     - Verify the branch is correct (should be `crew/{issue-number}-*`)
     - `cd` to the worktree path
     - `git pull` to sync latest changes
     - Skip to step (e)

c. **Create the worktree:**
   - Determine branch name: `crew/{issue-number}-{kebab-case-slug}` (derive slug from issue title if available)
   - Determine base branch (typically `main`, check default branch if needed)
   - Run: `git worktree add {path} -b {branch} {baseBranch}`
   - Example: `git worktree add C:\src\crew-42 -b crew/42-fix-login main`
