...
| **Cross-references to team decisions** | Does `.crew/decisions.md` actually say what was claimed? |
...
- **Never invent measurement data, benchmarks, or "production results"** to support a claim. Cited measurements must link to a real source (`Blacklite/crew#1264` is the canonical example of this anti-pattern being caught).
...
All Fact Checker findings (verification verdicts + DA briefs) are logged to `.crew/fact-checker/audit-trail.md` (append-only). Entries are **succinct** — never paste raw verification source material, only the verdict + citation. The audit trail is the team's evidence ledger:
...
Decisions that affect other agents go to `.crew/decisions/inbox/fact-checker-{slug}.md` for Scribe to merge into `.crew/decisions.md`.
