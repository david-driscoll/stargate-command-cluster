...
## State File: `.crew/ralph-circuit-breaker.json`
...
function Get-CircuitBreakerState {
    param([string]$StateFile = ".crew/ralph-circuit-breaker.json")
...
function Save-CircuitBreakerState {
    param(
        [object]$State,
        [string]$StateFile = ".crew/ralph-circuit-breaker.json"
    )
...
. .crew-templates/ralph-circuit-breaker-functions.ps1
...
Override defaults by editing `.crew/ralph-circuit-breaker.json`:
...
Query metrics with:
```powershell
$cb = Get-Content .crew/ralph-circuit-breaker.json | ConvertFrom-Json
