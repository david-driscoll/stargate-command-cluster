...
if ($Merge) {
    ...
    $namespaces = git -C $repo for-each-ref "refs/notes/crew/" --format="%(refname)" 2>&1
...
if (-not $Quiet) {
    $refs = git -C $repo for-each-ref "refs/notes/crew/" --format="%(refname)" 2>&1
    if ($refs) {
        ...
        Log "No crew notes yet." DarkGray
    }
}
