...
**⚡ Inline the charter.** Before spawning, read the agent's `charter.md` (resolve from team root: `{team_root}/.crew/agents/{name}/charter.md`) and paste its contents directly into the spawn prompt. This eliminates a tool call from the agent's critical path. The agent still reads its own `history.md` and `decisions.md`.
...
prompt: |
  ...
  YOUR CHARTER:
  {paste contents of .crew/agents/{name}/charter.md here}

  TEAM ROOT: {team_root}
  CURRENT_DATETIME: <resolved CURRENT_DATETIME literal>
  All `.crew/` paths are relative to this root.
  ...
  - Read-only project state: Do NOT write to project's .crew/ directory
  ...
  **WORKTREE:** You are working in a dedicated worktree at `{WORKTREE_PATH}`.
  ...
  Mutable crew state is owned by the runtime. You MUST use the `state.*` tools
  whenever they are available:
  - `crew_state_read` / `crew_state_list` for decisions, history, logs, and inbox entries
  - `crew_state_write` / `crew_state_append` for durable updates
  - `crew_state_delete` after Scribe merges inbox entries
  - `crew_state_health` when diagnosing backend availability
  - `crew_decide` for team-relevant decisions
  ...
  note refs, or write mutable `.crew/` state files by hand. Static config (charters,
  team.md, routing.md, skills) remains on disk and may be read with normal file tools.

  Read `agents/{name}/history.md` with `crew_state_read` when state tools are available; otherwise fall back to `.crew/agents/{name}/history.md`.
  Read `decisions.md` with `crew_state_read` when state tools are available; otherwise fall back to `.crew/decisions.md`.
  If .crew/identity/wisdom.md exists, read it before starting work.
  If .crew/identity/now.md exists, read it at spawn time.
  Check project skill directories (.crew/skills/, .github/skills/, .copilot/skills/, .claude/skills/, .agents/skills/) for any SKILL.md the coordinator attached to your prompt.
  ...
  - If .crew/identity/now.md has a `last_verified` timestamp older than your session
  ...
  **Requested by:** {current user name}
  ...
  1. APPEND learnings with `crew_state_append` to `agents/{name}/history.md`.
  ...
  2. If you made a team-relevant decision, call `crew_decide`. If that tool is
     unavailable, use `crew_state_write` to `decisions/inbox/{name}-{brief-slug}.md`.
