...
> **Status (v0.10.0):** This skill describes a **design proposal**, not a shipped runtime. Skill files install via `crew init`/`upgrade`, but the underlying tier scaffolding (`.crew/memory/hot/`, `cold/`, `wiki/`), Scribe promotion logic, and spawn-template tier-aware reads are tracked in [Blacklite/crew#1264](https://github.com/Blacklite/crew/issues/1264). Until those land, agents continue to load full `history.md` + `decisions.md` on every spawn.

## Overview

Crew agents today load their full context history on every spawn, which grows unboundedly across sessions. The Tiered Agent Memory model proposes a three-tier separation so agents only load the bytes that are actually relevant to the current task, with older context kept available on demand.
...
See: .crew/memory/cold/{agent-name}.md
```
...
Until then, see the Scribe charter for current behavior: `.crew/agents/scribe/charter.md`

---

## Implementation Checklist (tracked in #1264)

- [ ] Scribe writes Hot context file at session start (`.crew/memory/hot/{agent}.md`)
...
- [ ] Wiki entries stored in `.crew/memory/wiki/`
- [ ] Cold entries stored in `.crew/memory/cold/` with rolling TTL

---

## References

- Tracking issue: [Blacklite/crew#1264](https://github.com/Blacklite/crew/issues/1264) — installation gap + runtime status
- Original design spike: [Blacklite/crew#686](https://github.com/Blacklite/crew/issues/686) — tiered memory implementation plan
- Related: [Blacklite/crew#600](https://github.com/Blacklite/crew/issues/600) — context payload growth
...
Use this template when spawning any Crew agent. By default it loads **Hot tier only**. Add optional sections as needed.
...
`.crew/memory/cold/{agent-name}.md`
...
```
## Cold Memory Summary
{contents_of_.crew/memory/cold/{agent-name}.md}
```
...
Wiki entries are at: `.crew/memory/wiki/`
...
```
## Wiki Reference
{contents_of_.crew/memory/wiki/{topic}.md}
```
...
- Runtime backing is tracked in [Blacklite/crew#1264](https://github.com/Blacklite/crew/issues/1264) — until those changes land, this skill is design-only and agents continue to load full history.md + decisions.md on every spawn
