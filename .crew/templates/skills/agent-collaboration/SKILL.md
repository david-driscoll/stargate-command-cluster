...
description: "Standard collaboration patterns for all crew agents — worktree awareness, decisions, cross-agent communication"
...
### Worktree Awareness
Use the `TEAM ROOT` path provided in your spawn prompt. All `.crew/` paths are relative to this root. If TEAM ROOT is not provided (rare), run `git rev-parse --show-toplevel` as fallback. Never assume CWD is the repo root.

### Decision Recording
After making a decision that affects other team members, write it to:
`.crew/decisions/inbox/{your-name}-{brief-slug}.md`
...
- Don't write directly to `.crew/decisions.md` — always use the inbox drop-box
