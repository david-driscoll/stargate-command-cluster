...
Crew's coordinator prompt (`crew.agent.md`) and agent charters (e.g.
`scribe-charter.md`) are shipped as templates in `.crew-templates/`. Changes to
these files affect how every crew session behaves — but unit tests can't catch
...
crew sessions against a locally-built CLI that includes your modified templates.

## When To Use

- You changed `.crew-templates/crew.agent.md` (coordinator prompt)
- You changed `.crew-templates/scribe-charter.md` or other agent charters
- You changed `.crew-templates/notes-protocol.md` or helper scripts
...
- A local clone of the crew repo on your feature branch
...
|---|---|---|---|
| 1. Fast-fail checks (build :cd: link :cd: ``crew version``) | :hourglass_flowing_sand: Pending | --:-- | -- |
| 2. Create test repo(s) | :hourglass_flowing_sand: Pending | --:-- | -- |
| 3. ``crew init`` + file verification | :hourglass_flowing_sand: Pending | --:-- | -- |
...
```bash
cd /path/to/crew          # your feature branch
npm install
npm run build -w packages/crew-sdk && npm run build -w packages/crew-cli

# Link so `crew` command uses your local build (workspace flag — no cd required)
npm link -w packages/crew-cli
```

Verify: `crew version` output includes the `-preview` suffix (e.g., `x.y.z-preview`),
...
See [CONTRIBUTING.md — Making the `crew` Command Use Your Local Build](../../../CONTRIBUTING.md)
...
### Step 3 — Init a crew with your modified templates

```bash
crew init
# If testing a specific feature (e.g. state backends):
# crew init --state-backend git-notes
```

Verify the init produced the expected files:
```bash
ls -la .crew/
cat .crew/team.md          # should have ## Members with 3+ agents
cat .crew/config.json      # should reflect any CLI flags you passed
```
...
```powershell
# PowerShell (Windows)
copilot --agent crew --allow-all-tools -p "Picard, decide what testing framework to use. Write your decision." `
...
copilot --agent crew --allow-all-tools -p "prompt A" 2>&1 | Tee-Object evidence/session-A.log

# Session B: verify state persisted
copilot --agent crew --allow-all-tools -p "What decisions has the team made?" 2>&1 | Tee-Object evidence/session-B.log
```
...
git notes --ref=crew list              # git-notes backend
git ls-tree -r crew-state              # orphan backend
ls .crew/agents/*/history.md           # worktree backend
...
|---|---|---|---|
| 1. Fast-fail checks (build · link · \\`crew version\\`) | ⏳ Pending | --:-- | -- |
| 2. Create test repo(s) | ⏳ Pending | --:-- | -- |
| 3. \\`crew init\\` + file verification | ⏳ Pending | --:-- | -- |
...
1. Fast-fail checks (build · link · `crew version`)
2. Create test repo(s)
3. `crew init` + file verification
4. Run sessions
5. Verify outcomes
...
| Step | Status | Started | Duration |
|---|---|---|---|
| 1. Fast-fail checks (build · link · `crew version`) | :white_check_mark: Passed | $step1Started | $step1DurationText |
...
| `crew init` + file verification (no copilot session) | 3-5 min | 3 per agent |
| `crew init` + one `copilot --agent crew` session | 8-15 min | **1 per agent** |
| Build + link + one copilot session | 12-20 min | **1 per agent** |

**Rule: Limit yourself to 1 scenario that includes a `copilot --agent crew` session
...
**If you are running a scenario with a `copilot --agent crew` session:**
...
| 5 | Migration path | Existing crew picks up new template behavior | — |
...
0. **Clean stale SDK before building.** Before running `npm run build`, remove any
   stale published copy of `@blacklite/crew-sdk` that may have been installed
   into `packages/crew-cli/node_modules/`. This local copy shadows the workspace
   ...
   ```powershell
   $stale = "packages\crew-cli\node_modules\@bradygaster\crew-sdk"
   ...
   a no-op. The root `node_modules\@bradygaster\crew-sdk` workspace symlink remains
...
2. **CLI must link successfully.** `cd packages/crew-cli && npm link` must exit
   0. If it fails, report `LINK_FAILED` and stop.
3. **`crew version` must run.** After linking, `crew version` must output a
...
Example — ❌ wrong: `~\..\..\repos\crew\.e2e\pr-1035\evidence`
...
- **Running multiple `copilot --agent crew` sessions in one agent.** Each
...
This also applies when `copilot --agent crew` is launched as a subprocess from
...
```powershell
copilot --agent crew --allow-all-tools --allow-all-paths -p "..."
```
