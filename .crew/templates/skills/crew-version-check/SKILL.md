---
name: "crew-version-check"
description: "Internals of how @blacklite/crew-cli stamps its version, how `crew upgrade` works (what it preserves vs overwrites), and how to probe the npm registry for the latest version from a coordinator prompt."
allowedTools: []
confidence: medium
domain: crew-internals
source: "Discovered by Data; validated in Blacklite/crew#1173 recon (2026-05-26)."
---
...
**Discovered by:** Data
**Date:** 2026-05-26
**Validated in:** Issue #1173 recon (Blacklite/crew)

---

## What This Skill Covers

Reusable knowledge about how `@blacklite/crew-cli` stamps its version into `crew.agent.md`, how `crew upgrade` works, what it preserves vs. overwrites, and how to probe the npm registry for the latest version from a coordinator prompt.

---

## Package & Registry Facts

- **Package name:** `@blacklite/crew-cli`
- **Registry:** npm (public)
- **CLI binary:** `crew` (registered via `package.json#bin.crew`)
...
## `crew upgrade` Behavior

**Source file:** `dist/cli/core/upgrade.js`

### What gets overwritten:
- `crew.agent.md` — full overwrite from template, then `stampVersion()`
...
- `.crew/config.json` — **never touched**; `stateBackend` survives intact
...
- npm: `npm install -g @blacklite/crew-cli@latest`
- pnpm: `pnpm add -g @blacklite/crew-cli@latest`
- yarn: `yarn global add @blacklite/crew-cli@latest`
...
## `.crew/config.json` — What It Holds
...
```
npm view @blacklite/crew-cli dist-tags --json
```
...
```
node -e "const p=require('path'),o=require('os');const b=process.env.APPDATA||(process.platform==='darwin'?p.join(o.homedir(),'Library','Application Support'):p.join(o.homedir(),'.config'));const f=p.join(b,'crew-cli','update-check.json');try{const d=JSON.parse(require('fs').readFileSync(f,'utf8'));const age=Date.now()-d.checkedAt;if(age<86400000)console.log(JSON.stringify(d));else console.log('STALE')}catch{console.log('MISS')}"
```
...
- Windows: `%APPDATA%\crew-cli\update-check.json`
- Linux: `~/.config/crew-cli/update-check.json`
- macOS: `~/Library/Application Support/crew-cli/update-check.json`

---

## Repo-Local Cache Convention: `.crew/.cache/version-check.json`
...
**Gitignore:** `.crew/.cache/` is listed in `.gitignore` — cache files are never committed.
...
| crew.agent.md template | `templates/crew.agent.md.template` |
