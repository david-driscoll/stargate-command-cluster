...
Check whether a newer Crew version exists for the user's channel. Append to
the greeting if a newer version is found. Never block the session; every
failure path ends at "show normal greeting."

### 1.1 Kill Switch

If the environment variable `CREW_NO_UPDATE_CHECK` is set to `1`, **skip
...
top of `crew.agent.md` (or from the `- **Version:** X` identity line as
...
The upstream Crew CLI (`self-update.ts`) already fetches the latest version
...
Read `.crew/.cache/version-check.json`. If the file exists, is not older than
...
- On success: extract `dist-tags[channel]` (e.g., `dist-tags["insider"]`).
  Write `.crew/.cache/version-check.json` with the schema above.
  Create `.crew/.cache/` if it does not exist.
...
```
 · 🆕 v{latestVersionForChannel} available — say "upgrade crew"
```

Example complete greeting line:
```
Crew v0.9.4-insider.1 · 🆕 v0.9.7-insider.1 available — say "upgrade crew"
```
...
1. **Confirm** — ask the user to confirm before running the upgrade:
   > "I'll run `crew upgrade` now. This overwrites `crew.agent.md` and
...
3. **What's-new digest** — after successful upgrade, fetch and summarize
   ...
   ```
   gh api repos/Blacklite/crew/releases --jq '[.[] | select(.tag_name | test("^v"))]'
   ```
   ...
   - **Fallback chain:**
     - `gh` not authenticated → "See full release notes at:
       https://github.com/Blacklite/crew/releases"
...
4. **Restart prompt** — after showing the digest, prompt the user:
   > "`crew.agent.md` has been updated. For the new coordinator instructions
...
| `.crew/.cache/` write error | Normal greeting (skip cache write) |
| `gh` not available / unauthenticated | Upgrade flow: link to releases page |
| `crew upgrade` exits non-zero | Report error, stop flow |
| Any unexpected exception | Log to `.crew/orchestration-log/`, normal greeting |
