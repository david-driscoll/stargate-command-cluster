...
$GARAGE_CMD key import -n postgres-sgc --yes "$GARAGE_USER_POSTGRES_SGC" "$GARAGE_PASSWORD_POSTGRES_SGC" || true
$GARAGE_CMD bucket create postgres-${CLUSTER_CNAME} || true
$GARAGE_CMD bucket allow --read --write --owner postgres-${CLUSTER_CNAME} --key postgres-sgc
$GARAGE_CMD bucket allow --read --write postgres-${CLUSTER_CNAME} --key cluster-user
