...
description: Never read .env files or write secrets to .crew/ committed files
...
Spawned agents have read access to the entire repository, including `.env` files containing live credentials. If an agent reads secrets and writes them to `.crew/` files (decisions, logs, history), Scribe auto-commits them to git, exposing them in remote history. This skill codifies absolute prohibitions and safe alternatives.
...
**NEVER write these to `.crew/` files:**
...
**Before committing `.crew/` changes, Scribe MUST:**
...
3. **If secrets detected:**
   ...
   - Report to user:
     ...
     File: .crew/decisions/inbox/river-db-config.md
...
2. **Alert the user:**
   ...
   Commit: abc1234
   File: .crew/decisions/inbox/agent-config.md
...
→ Writes to .crew/decisions/inbox/river-env-setup.md:
...
→ Writes to .crew/decisions/inbox/river-db-schema.md:
...
- ❌ Writing secrets to "temporary" files in `.crew/` — Scribe commits ALL `.crew/` changes
