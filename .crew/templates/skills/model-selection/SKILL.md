...
- Persistent model preferences in `.crew/config.json`
...
Crew supports 18+ models across three tiers (premium, standard, fast). The coordinator must select the right model for each agent spawn. Users can set persistent preferences that survive across sessions.
...
|-------|------|--------|-------------|
| **0a** | Per-Agent Config | `.crew/config.json` → `agentModelOverrides.{name}` | Persistent (survives sessions) |
| **0b** | Global Config | `.crew/config.json` → `defaultModel` | Persistent (survives sessions) |
...
2. WRITE `defaultModel` to `.crew/config.json` (merge, don't overwrite)
...
1. VALIDATE model ID
2. WRITE to `agentModelOverrides.{agent}` in `.crew/config.json`
...
1. REMOVE `defaultModel` from `.crew/config.json`
...
- Create new config files (only modify existing `.crew/config.json`)
- Change the model after spawn (fallback chains handle runtime failures)

## Config Schema

`.crew/config.json` model-related fields:
