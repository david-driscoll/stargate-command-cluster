...
- `economyMode: true` written to `.crew/config.json` when activated persistently
...
| Persistent config | `"economyMode": true` in `.crew/config.json` |
| CLI flag | `crew --economy` |
...
1. WRITE `economyMode: true` to `.crew/config.json` (merge, don't overwrite other fields)
...
1. REMOVE `economyMode` from `.crew/config.json` (if it was persisted)
...
`.crew/config.json` economy-related fields:
