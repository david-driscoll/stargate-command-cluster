...
- A crew member hands you review comments to resolve
...
gh api repos/Blacklite/crew/pulls/42/comments/18234/replies \
  -f body="Fixed in a1b2c3d — switched to path.dirname(crewDirInfo.path) for worktree consistency"

# When applying a suggested code change
gh api repos/Blacklite/crew/pulls/42/comments/18235/replies \
...
gh api repos/Blacklite/crew/pulls/42/comments/18236/replies \
  -f body="Considered but not applied — this path needs to stay absolute because worktree resolution depends on it. See detectCrewDir() in detect-crew-dir.ts."
```
...
> `crewDir` could be undefined here. Consider adding a null check.

**Agent workflow:**
1. Read the comment via `get_review_comments`
2. Add the null check in `src/cli/core/detect-crew-dir.ts`
3. Commit: `fix: add null check for crewDir (PR #99 review)`
4. Reply:
   ```bash
   gh api repos/Blacklite/crew/pulls/99/comments/55123/replies \
     -f body="Fixed in f4e5d6c — added early return when crewDir is undefined, matching the pattern in loadConfig()"
   ```
...
**Comments:**
- id: 55123 — "Null check needed" on `detect-crew-dir.ts:42`
- id: 55124 — "Consider using path.join()" on `detect-crew-dir.ts:58`
- id: 55125 — "This log message is too verbose" on `output.ts:15`

**Agent handles each individually:**
```bash
# Fix all three, commit
git add packages/crew-cli/src/cli/core/detect-crew-dir.ts packages/crew-cli/src/cli/core/output.ts
git commit -m "fix: address 3 review comments on PR #99

- Added null check for crewDir (comment #55123)
...
gh api repos/Blacklite/crew/pulls/99/comments/55123/replies \
  -f body="Fixed — added early return when crewDir is undefined"

gh api repos/Blacklite/crew/pulls/99/comments/55124/replies \
  -f body="Fixed — switched to path.join(crewDir, 'config.json') for cross-platform consistency"

gh api repos/Blacklite/crew/pulls/99/comments/55125/replies \
...
```bash
gh api repos/Blacklite/crew/pulls/99/comments/55130/replies \
...
```bash
gh api repos/Blacklite/crew/pulls/99/comments/55140/replies \
