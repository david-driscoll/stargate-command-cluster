...
prompt: |
  ...
  Tasks (in order):
  0. PRE-CHECK: Run `crew_state_health` when available. If state tools are unavailable,
  ...
  2. DECISION INBOX: Use `crew_state_list` and `crew_state_read` on `decisions/inbox`,
     merge entries into `decisions.md` with `crew_state_write`, delete processed inbox
     entries with `crew_state_delete`, and deduplicate.
  3. ORCHESTRATION LOG: Write `orchestration-log/{timestamp}-{agent}.md` with `crew_state_write` per agent. Use ISO 8601 UTC timestamp. Replace `:` with `-` in `{timestamp}` so filenames are valid on all platforms (e.g. `2026-06-02T21-15-30Z`).
  4. SESSION LOG: Write `log/{timestamp}-{topic}.md` with `crew_state_write`. Brief. Use ISO 8601 UTC timestamp. Replace `:` with `-` in `{timestamp}` so filenames are valid on all platforms.
  ...
  7. HEALTH REPORT: Log decisions.md before/after size, inbox count processed, history files summarized with `crew_state_write` or `crew_state_append`.

  Runtime state tools own persistence. Never switch branches, push note refs, reset
  `.crew/`, or commit mutable crew state from this prompt.
