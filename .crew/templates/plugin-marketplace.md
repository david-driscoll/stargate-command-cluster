...
Registered marketplace sources are stored in `.crew/plugins/marketplaces.json`:
...
- `crew plugin marketplace add {owner/repo}` — Register a GitHub repo as a marketplace source
- `crew plugin marketplace remove {name}` — Remove a registered marketplace
- `crew plugin marketplace list` — List registered marketplaces
- `crew plugin marketplace browse {name}` — List available plugins in a marketplace

## When to Browse

During the **Adding Team Members** flow, AFTER allocating a name but BEFORE generating the charter:

1. Read `.crew/plugins/marketplaces.json`. If the file doesn't exist or `marketplaces` is empty, skip silently.
...
2. Copy it into the agent's skills directory: `.crew/skills/{plugin-name}/SKILL.md`
