---
name: reflect
description: Learning capture system that extracts HIGH/MED/LOW confidence patterns from conversations to prevent repeating mistakes. Use after user corrections ("no", "wrong"), praise ("perfect", "exactly"), or when discovering edge cases. Complements .crew/agents/{agent}/history.md and .crew/decisions.md.
license: MIT
version: 1.0.0-crew
...
**Critical learning capture system** for Crew. Prevents repeating mistakes and preserves successful patterns across sessions.

Analyze conversations and propose improvements to crew knowledge based on what worked, what didn't, and edge cases discovered. **Every correction is a learning opportunity.**

---

## Integration with Crew Architecture

**Reflect complements existing Crew knowledge systems:**

1. **`.crew/agents/{agent}/history.md`** — Permanent learnings from completed work (append-only; each agent updates their own file; Scribe propagates cross-agent updates)
2. **`.crew/decisions.md`** — Team-wide decisions that all agents respect
...
1. **Agent-specific learning** → `.crew/agents/{agent}/history.md`
2. **Team-wide decision** → `.crew/decisions/inbox/{agent}-{topic}.md`
...
│   Target: .crew/decisions/inbox/{agent}-{topic}.md         │
│                                                             │
│ [MED]  + Add preference: "{specific preference}"            │
│   Source: "{evidence from conversation}"                    │
│   Target: .crew/agents/{agent}/history.md                  │
│                                                             │
...
[MED] + Add preference: "Include test plan in PR descriptions"
  ...
  Target: .crew/decisions/inbox/pr-format.md (for team adoption)
```
...
- `.crew/decisions.md` — Team-wide decisions
- `.crew/agents/*/history.md` — Agent-specific learnings
- `.crew/routing.md` — Work assignment patterns
